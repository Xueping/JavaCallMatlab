function [ cascade_id cascade_time data_rate I_cas I_rate cas_len num_item num_node T_window sparsity] = datareform_movielens(data,longestlen)
...
nn=unique(data(:,1));
ii=unique(data(:,2));
num_node=length(nn);%max(data(:,1));
num_item=length(ii);%max(data(:,2));
T_window=max(data(:,4));

for i=1:length(nn)
    index=find(data(:,1)==nn(i));
    data(index,1)=i;
end

for j=1:length(ii)
    index=find(data(:,2)==ii(j));
    data(index,2)=j;
end

cascade_id=zeros(longestlen,num_item);
cascade_time=zeros(longestlen,num_item);
cas_len=[];
for i=1:num_item
    index=find(data(:,2)==i);
    li=length(index);
...
n40=length(find(data_rate==4))+length(find(data_rate==4.5));
n30=length(find(data_rate==3))+length(find(data_rate==3.5));
n20=length(find(data_rate==2))+length(find(data_rate==2.5));
n10=length(find(data_rate==1))+length(find(data_rate==1.5))+length(find(data_rate==0.5));
...
% save Data_bag_u1base cascade_id cascade_time data_rate I_cas I_rate
% cas_len num_item num_node T_window sparsity; 